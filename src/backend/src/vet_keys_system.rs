...
pub async fn vetkd_public_key(request: VetKDPublicKeyRequest) -> VetKDPublicKeyReply {
    ...
    let flattened_path: Vec<u8> = request
        .derivation_path
        .iter()
        .flat_map(|v| v.iter().cloned())
        .collect();
    let derivation_context = DerivationContext::new(canister_id.as_slice(), &flattened_path);
    let derived_public_key = DerivedPublicKey::compute_derived_key(&MASTER_PK, &derivation_context);
    VetKDPublicKeyReply {
...
pub async fn vetkd_derive_encrypted_key(
    request: VetKDEncryptedKeyRequest,
    ...
    let flattened_path: Vec<u8> = request
        ...
        .flat_map(|v| v.iter().cloned())
        .collect();
    let derivation_context = DerivationContext::new(ic_cdk::caller().as_slice(), &flattened_path);
    let tpk =
